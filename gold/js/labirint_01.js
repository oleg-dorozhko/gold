...
var glob_sound_off = true;

// window.onload = function()
// {
	
	// CLIPBOARD_CLASS("center_canvas", true); //not works not checheked not tested
...
function whenUserLeftClickOnLeftCanvas(e)
{
	...
	else
	{
		...
		if(result==1) global_last_selected_object = null;
	}
...
function user_move(obj1, obj2)
{
	...
	{
		if(all_checked) return move_to_all_back(obj1, obj2);
	...
	else if ( (obj1.frm.indexOf("left_canvas")!= -1 ) && (obj2.frm.indexOf("left_canvas")!= -1 ) )
	{
		if((n1 == n2) && (m1==m2))
		{
			if(all_checked) return move_to_all(obj1);
			else return move_to(obj1);
		}
		
		if( isImageDataCellPatterned(imgData1) && isImageDataCellPatterned(imgData2) )
		{
			if(cmp(imgData1, imgData2)) return -1;
			
			if(all_checked) return swap_all(obj1, obj2, true);
			else return swap(obj1, obj2, true);
		}
