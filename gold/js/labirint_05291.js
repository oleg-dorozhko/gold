...
function setSomePatternedOnThisColored(obj)
{
	...
	for(var i=0;i<glob_koloda.length;i++)
	{
		...
		if(cmp(imgData,imgData27))
		{
			...
			var canvas = document.getElementById("left_canvas");
			var context = canvas.getContext("2d");
			context.putImageData(imgData27, obj.nm[2]*global_seed_size, obj.nm[3]*global_seed_size);
			return true;
...
function magik()
{
	...
	do
	{
		...
		{			
			...
			if( res == true ) take_a_fifty_colored ();
...
function take_a_fifty()
{
	...
	for(var i=0;i<(list.length/2|0);i++)
	{
		...
		{
			var el = list[i];
			var result  = move_to_rnd_back(el.getContext("2d").getImageData(0,0,el.width,el.height));
			if(result!=null) 
			{
				remove_kard(el);
...
function prepare_koloda()
{
	...
	shuffle(document.getElementById("first_kard").childNodes);
...
function getUniqKardId()
{
	...
	var list = document.getElementById("first_kard").childNodes;
	var cnv = document.getElementById("left_canvas");
	var whlim = cnv.width/global_seed_size*cnv.height/global_seed_size;
	var nn=0;
	for(var i=0;i<list.length;i++)
	{
		
		if(list[i].id.indexOf("kard_"+nn)==-1)
		{
			return "kard_"+nn;
		}
...
function move_to_rnd_back2(obj1, obj2)
{
	var arr = null;
...
function shoot(arr_gun4)
{
	...
	//	if(document.getElementById("qflag1").checked == true ) all_checked=true;
		...
		if ( (obj1.frm.indexOf("kard_")!= -1 ) && (obj2.frm.indexOf("left_canvas")!= -1 ) )
		{
			...
			var result =  move_to_rnd_back2(obj1, obj2);
			if(result != null)
			{
