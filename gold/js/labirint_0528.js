...
function whenUserLeftClickOnLeftCanvas(e)
{
	...
	var obj = get_selected_object(e);
	if((check_own_place2(obj)==true) && (arr_gun.length==0)) { soundKosh(); return; }
...
function recursive_shoot( callback )
{
		...
		setTimeout( function() { recursive_shoot(); }, 300 );
...
function get_half_colored()
{
	if(arr_gun.length == 3)
	{
		...
		if( cmp (imgDataF, imgDataS) ) return;
		if( cmp (imgDataF, imgDataT) ) return;
		if( cmp (imgDataS, imgDataT) ) return;
