...
function mod_cli_up_upForImageData(im)
{
	return mod_cli_up_up_image_data( im, 1 );
}
...
function mod_cli_up_up_image_data( imageData, up_counter ){
...
var	im0 = __node__getPartOfImageData(imageData,0,0,up_counter,up_counter);	//context.getImageData(0,0,up_counter,up_counter);
	...
	var imageData2 = getNewImageData(w,h);
	...
	im = mod_cli_mirror_mirror_right(__node__getPartOfImageData(imageData2,0,0,nw,nh));
	//im = mirror_right(imageData2);
	im = mod_cli_mirror_mirror_down(im);
	//im = mirror_down(im);
