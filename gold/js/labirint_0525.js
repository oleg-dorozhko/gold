...
function move_to(obj,last_line)
{
	...
	var color = getMappedColor(n,m);//addImgDataToGalerka(n,m,imgData);
	//if(color!=null) 
	{	
		context.fillStyle = color;
		context.fillRect(n*global_seed_size,m*global_seed_size,global_seed_size,global_seed_size);
...
function move_to_all_back(obj1,obj2)
{
	...
	if((obj1.frm.indexOf("galerka_") == -1) || (obj1.frm.indexOf("kard_") == -1)) return -1;
...
function next_kard()
{
	if(glob_selected_kard==-1) return;
	
	document.getElementById("first_kard").removeChild(glob_koloda[glob_selected_kard]);
	
	glob_koloda[glob_selected_kard] = null;
	
	if(glob_selected_kard==0) glob_selected_kard = glob_koloda.length;
	
	glob_selected_kard--;
	
	if(glob_koloda[glob_selected_kard]==null)
	{
		var found = null;
		var counter = 0;
		var n = glob_selected_kard;
		var nn = 0;
		while(true)
		{
			if( n == 0 ) n = glob_koloda.length;
			
			n--;
			
			if(glob_koloda[n] != null) { found = n; break; }
			
			nn++;
			
			if(nn > glob_koloda.length) break;
		}
		
		if(found == null) 
		{
			alert('cards not found');
			glob_selected_kard=-1;
			return;
		}
		
		glob_selected_kard = n;
	}
	...
	document.getElementById("first_kard").appendChild(cnv);
...
function put_kard_to_koloda(obj)
{
...
function whenUserClickedGalerka(e)
{
	...
	if(e.target.parentElement == document.getElementById("first_kard")) 
	{
		if(getGalerkaCanvasNumber() < 8 ) moveFirstKardOnGalerka();
...
function shoot()
{
	...
	//	if(document.getElementById("qflag1").checked == true ) all_checked=true;
		...
		if( (obj1.frm.indexOf("kard")!= -1 ) && (obj2.frm.indexOf("left_canvas")!= -1 ) )
		{
			
			if(all_checked) 
			{
				/****
				result = move_to_all_back(obj1, obj2);
